...
class _ManualStarScreenState extends State<ManualStarScreen> {
  ...
  Future<List<Map<String, dynamic>>> _fetchUsers() async {
    try {
      final response = await ApiService.get('/admin/manual-star-users');
      if (response.statusCode == 200) {
        final data = json.decode(response.body);
        return List<Map<String, dynamic>>.from(data is List ? data : (data['data'] ?? []));
      } else {
        final errorData = json.decode(response.body);
        throw 'Failed to load users for manual star: ${errorData['message'] ?? 'Unknown error'}';
      }
    } catch (e) {
  ...
  Future<void> _setScore(String userId, int score) async {
    try {
      await ApiService.post(
          '/admin/set-manual-star-score',
          {'p_user_id': userId, 'p_score': score});
